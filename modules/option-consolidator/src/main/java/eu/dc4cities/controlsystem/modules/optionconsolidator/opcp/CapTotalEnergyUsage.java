 ...
 *
 */
public class CapTotalEnergyUsage extends CappableConstraint {
     ...
     * @param k    the maximum amount of energy to use for the period in Watt hour
     */
    public CapTotalEnergyUsage(int from, int to, int k) {
        super("<=");
        wh = k;
        this.from = from;
        this.to = to;
    ...
    public void decorate(OPCP pb) {
        ...
        pb.getSolver().post(ICF.arithm(energy, getOperator(), wh));
    }

    @Override
    public String toString() {
        return "energy([" + from + ":" + to + "[) " + getOperator() + wh;
    }
